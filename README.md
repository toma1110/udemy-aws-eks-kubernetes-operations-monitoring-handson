...
7. Section 8は、[初動対応Runbookとコスト・削除確認](labs/s8-operations-runbook/README.md)を入口にします。東京RegionのAWS CloudShell Bashから共通EKS基盤を読み取り専用で観察し、Runbook、料金発生源、所有権、cleanup順序を確認します。
8. 許可された既存EKSクラスターを読むだけの場合は、[読み取り専用の状態確認](scripts/collect_readonly_evidence.ps1)を利用できます。既存リソースを教材の削除対象にしません。
